...
import com.bajarmusica.descargarmusicarapidoalcelularguide.ypylibs.utils.DownloadUtils;
import com.bajarmusica.descargarmusicarapidoalcelularguide.ypylibs.utils.StringUtils;
...
public class XRadioNetUtils implements IXRadioConstants {
    ...
    private static final String FORMAT_LAST_FM_V2 = "http://ws.audioscrobbler.com/2.0/?method=track.getInfo&track=%1$s&artist=%2$s&api_key=%3$s&format=json&limit=1";
    ...
    public static String getImageOfSong(String title, String artist, String apiKey) {
        try {
            if (!TextUtils.isEmpty(title) && !TextUtils.isEmpty(artist)) {
                String searchTitle = StringUtils.urlEncodeString(title);
                String searchArtist = StringUtils.urlEncodeString(artist);
                String url = String.format(FORMAT_LAST_FM_V2, searchTitle, searchArtist, apiKey);
                YPYLog.e("DCM", "====>search url=" + url);
                return JsonParsingUtils.parsingImageSong(DownloadUtils.downloadString(url));
            }
        }
        catch (Exception e) {
            e.printStackTrace();
